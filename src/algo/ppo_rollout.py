...
class PPORollout(BaseAlgorithm):
    ...
    def __init__(
        ...
        # self.pretrain = policy_kwargs['pretrain']
    ...
    def on_training_start(self):
        ...
        self.global_episode_true_rewards = np.zeros(self.n_envs, dtype=np.float32)
    ...
    def clear_on_episode_end(self, dones, policy_mems, model_mems):
        for env_id in range(self.n_envs):
            if dones[env_id]:
                ...
                self.global_episode_true_rewards[env_id] = 0
    ...
    def log_on_rollout_end(self, log_interval):
        if log_interval is not None and self.iteration % log_interval == 0:
            log_data = {
                ...
                "time/fps": int(self.num_timesteps / (time.time_ns() - self.start_time)),
                "time/time_elapsed": int(time.time_ns() - self.start_time),
    ...
    def collect_rollouts(
        ...
        n_rollout_steps: int,
        reward_model: Optional[Type[MLPEnsemble]] = None,
    ) -> bool:
    ...
    def learn(
        ...
        if eval_env is not None and eval_freq > 0:
            ...
            callback = [callback, eval_callback] if callback is not None else eval_callback
