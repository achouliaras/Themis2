...
def worker_task(loop_id, video_id, config, policy_kwargs):
        """
        Each process receives its specific ID and Video ID.
        loop_id: the 0, 1, 2... index
        video_id: the specific identifier for the filename
        """

        wrapper_class = config.get_wrapper_class()
        env = config.get_env(wrapper_class, seed=config.run_id*12345 + video_id if config.fixed_seed >= 0 else None)
        ...
        ptr_str = f"Generating trajectory {loop_id}/{config.episode_num}: "

        writer = imageio.get_writer(
            f"{config.log_dir}/traj_videos/traj{config.run_id:02}{video_id:03}.mp4",
            fps=config.fps,
            codec="libx264",
            quality=8
        ) 
        if config.gen_xai_videos:
            xai_writer = imageio.get_writer(
                f"{config.log_dir}/traj_xai_videos/xai{config.run_id:02}{video_id:03}.mp4",
                fps=config.fps,
                codec="libx264",
                quality=8
            )
        ...
        last_policy_mems = float_zeros([1, model.policy.gru_layers, model.policy.dim_policy_features])

        done = False
        i=0
        while not done and i < config.video_length:
            with th.no_grad():
                obs_tensor = obs_as_tensor(last_obs, model.device)
                # action, policy_mem = model.predict(last_obs, state=last_policy_mems, deterministic=True)
                action, value, log_prob, policy_mem = model.policy.forward(obs_tensor, last_policy_mems) 
                action = action.cpu().numpy()
            ...
            next_obs, reward, done, info = env.step(clipped_action)
            frame = env.render()
            
            writer.append_data(frame)
            if config.gen_xai_videos:
                xai_writer.append_data(frame) # TODO: Add saliency maps generation

            last_obs_list.append(last_obs)
            next_obs_list.append(next_obs)
            last_policy_mems_list.append(last_policy_mems.clone().cpu().numpy())
            action_list.append(action)
            reward_list.append(reward)
            episode_starts_list.append(i == 0)
        ...
        data = {
            "last_obs": np.array(last_obs_list, dtype=np.uint8),
            "next_obs": np.array(next_obs_list, dtype=np.uint8),
            "last_policy_mems": np.array(last_policy_mems_list, dtype=np.float32),
            "action": np.array(action_list),
            "reward": np.array(reward_list, dtype=np.float32), # Downcast to save space
            "episode_starts": np.array(episode_starts_list, dtype=bool),
            "done": np.array(done_list, dtype=bool)
        }

        np.savez_compressed(f"{config.log_dir}/traj_data/traj{config.run_id:02}{video_id:03}.npz", **data)
        return f"{ptr_str} Trajectory {video_id+1} completed. Length: {len(reward_list)}."
