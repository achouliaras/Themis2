...
def train(config):
    ...
    # Save trainer kwargs for later loading of model checkpoints, while removing the env key which cannot be pickled.
    kwargs_to_save = trainer_kwargs.copy()
    if "env" in kwargs_to_save:
        del kwargs_to_save["env"]
    with open(f"{config.log_dir}/trainer_kwargs.pkl", "wb") as file:
            pickle.dump(kwargs_to_save, file)
            print("trainer_kwargs saved to trainer_kwargs.pkl")
    ...
    if config.pretrain_percentage > 0.0:
        ...
        if not matching_files:
            ...
            if config.reward_learning_frequency >= config.total_steps or config.reward_learning_frequency == -1 and config.curr_iter > 0:
                ...
                match = re.search(r'/train_models/train_model_(\d+)', os.path.basename(latest_model_path))
                if match:
                ...
                print(f'Pretrained model loaded from: {latest_model_path} with {num_timesteps_completed} pretrain steps completed.')
...
@click.option('--rm_rew_coef', default=0.0, type=float, help='Coefficient of reward model rewards')
